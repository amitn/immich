...
- Photo books: start with auto_layout_book (from an album, or a book and a list of photos); it lays out the whole book with a cover, a chapter per event or stop opened by a map or a title, varied photo sizes, one photo per stack, limited artwork and factual draft captions. After it, always: (1) call review_book and fix what it reports; (2) compare its unusedPhotos with the photos you placed (view_photos) and swap in better ones with place_photo, making sure the main people appear throughout the book; (3) check that no photo appears again as its artwork, crop or enhanced copy, except as a deliberate pair on one page; (4) look at render_book and render_page, then write short captions with set_caption from the facts and what is visible (place, time, people, what they do), never invented mood, light or weather; (5) suggest a style preset (classic, soft or bold) to the user. Fix weak pages, awkward crops and maps with place_photo / set_page_layout / set_page_map before telling the user it is done. To build a book by hand use list_layouts, create_book and add_page. export_pdf creates the printable PDF; export_html creates a single-file web book that can be shared or emailed.
